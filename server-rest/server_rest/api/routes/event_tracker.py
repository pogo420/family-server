...
router = APIRouter(prefix="/event-tracker", tags=["event tracker"])


@router.get("/", response_model=list[EventResponse])
def show_events(event_filter: EventFilter = Depends(), db: Session = Depends(get_db)):
...
def show_all_events(db: Session = Depends(get_db)):
